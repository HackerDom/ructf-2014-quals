...
В обработчике исключения идет asm мясо. Смысл:
...
4) ret прыгает по lbl_call_eax + 3. т.е прыгает в середину инструкции `mov eax, 0xD0FF5B21`, и выполняется на самом деле `FF D0` == call eax (call check_pass)
Функция check_pass:
...
Часть с upx:
Я сжал откомпиленный файл (причем компилировал без релоков и ASLR), вручную в hex editor добавил на место нулей в конце секции опкоды.
