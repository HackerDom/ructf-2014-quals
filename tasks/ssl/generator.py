...
client_random = "DiHe 1337 1337 1337 1337 1337 13"
...
code = "gslriwpx94auwcd9tu63jorqz6gutk9"

req = ("GET /secret/%s HTTP/1.1\n" % code +
       "Host: onetimesecret.com\n"
       "User-Agent: Bay browser 0.1\n\n")

encrypted_msg = gen_encrypted_appdata_msg(encryptor, client_seq_num, client_write_mac_key, req)
...
while True:
    ...
    if rec_type == APPLICATION_DATA:
        ...
        print("Got: %s" % msg)
        if "View Secret" in msg:
            print("View Secret")
            session = re.search("sess=([^;]+?);", msg).group(1)
            shrimp = re.search("var shrimp = '([^']+?)'", msg).group(1)
            # rj7vtlsgdiaao8j0hisnvoyqw4ngkiuc7ozm2m15f4scaldopwi3a5som21limigk8yvifcpj4hgbr6m99vgic28ijuh4tnjdxu'

            print("Session = " + session)
            print("shrimp = " + shrimp)

            content = "shrimp=%s&continue=true" % shrimp

            req = ("POST /secret/%s HTTP/1.1\n" % code +
                   "Host: onetimesecret.com\n"
                   "User-Agent: Bay browser 0.1\n"
